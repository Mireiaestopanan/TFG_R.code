...
dades_in$IS <- cut(dades_in$sportrec.hagos, breaks = c(0, 61, 84, 100), labels = c("stop", "attention", "go"))
dades_in$IS_num <- cut(dades_in$sportrec.hagos, breaks = c(0, 61, 84, 100), labels = c(3, 2, 1))
dades_in$IS_num <- as.numeric(dades_in$IS_num)
#class(dades_in$IS_num)

################# MODELLING Using covariable groin.problem.prev ##############################################
mod3 <- clmm(IS ~ groin.problem.prev + (week) + (1 | week) + (1 | id.player), data = dades_in)
summary(mod3)

#Checking proportional odds assumption for time
f <- lrm(IS ~ rcs(week, 5), data=dades_in)
...
windows()
propsPO(IS ~ week, odds.ratio=dor, data=dades_in) +
...
s <- summary(IS_num ~ groin.problem.prev + week, fun=sf, continuous=99, data=dades_in)
windows()
...
with(dades_in, print(table(week, IS)))


## 2) Running two separate logistic regressions. Calculate the ORs for the differemt collapsing levels
# Collapse first and second categories of IS
dades_in$IS_collapsed1 <- ifelse(dades_in$IS == "stop", 0, 1)
dades_in$IS_collapsed1 <- ifelse(dades_in$IS == "attention", 0, dades_in$IS_collapsed1)
# Fit model with collapsed categories
dades_in$IS_collapsed1<- factor(dades_in$IS_collapsed1)
mod_collapsed1 <- clmm(IS_collapsed1 ~ groin.problem.prev + (week) + (1 | week) + (1 | id.player), data = dades_in)
...
# Collapse second and third categories of IS
dades_in$IS_collapsed2 <- ifelse(dades_in$IS == "attention", 1, 0)
dades_in$IS_collapsed2 <- ifelse(dades_in$IS == "go", 1, dades_in$IS_collapsed2)
# Fit model with collapsed categories
dades_in$IS_collapsed2<- factor(dades_in$IS_collapsed2)
mod_collapsed2 <- clmm(IS_collapsed2 ~ groin.problem.prev + (week) + (1 | week) + (1 | id.player), data = dades_in)
# Obtain coefficient estimates and standard errors
...
## 1)
s2 <- summary(IS_num ~ tl.vs.no.tl.gproblem + factor(week), fun=sf, continuous=99, data=dades_in)
windows()
plot(s2, which=1:2, pch=0:4, xlab='logit Cumulative Prob', main='')

## 2)
mod_collapsed11 <- clmm(IS_collapsed1 ~ tl.vs.no.tl.gproblem + (week) + (1 | week) + (1 | id.player), data = dades_in)
# Obtain coefficient estimates and standard errors
...
# Collapse second and third categories of IS
# Fit model with collapsed categories
mod_collapsed22 <- clmm(IS_collapsed2 ~ tl.vs.no.tl.gproblem + (week) + (1 | week) + (1 | id.player), data = dades_in)
# Obtain coefficient estimates and standard errors
